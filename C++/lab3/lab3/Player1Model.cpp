...
namespace Game {
	...
	std::vector<std::pair<std::wstring, int>> Player1Model::_read_stat(std::wifstream& file) const {
	...
	void Player1Model::_circle_ship(std::vector<std::vector<int>>& field, int y, int x, int size) {
		int i = 1;
		while (true) {
			if (x - i >= 0) {
				if (field[x - i][y] == 8) i++;
	...
	void Player1Model::set_field(const std::vector<std::vector<int>> field) {
	...
	std::vector<std::vector<int>> Player1Model::get_field() const {
		return _player_field;
	}

	std::vector<std::vector<int>> Player1Model::get_enemy_field() const {
		return _enemy_field;
	}

	void Player1Model::set_name(const std::wstring name) {
		_name = name;
	}

	void Player1Model::write_statistic() {
	...
	bool Player1Model::check_move(int pos_x, int pos_y) {
		if (_enemy_field[(pos_y - 124) / 41][(pos_x - 788) / 41] != 0) return false;
		return true;
	}

	bool Player1Model::make_move(int pos_x, int pos_y, int type, int size) {
		if (type == 1) {
	...
	void Player1Model::take_enemy_move(int pos_x, int pos_y, int type, int size) {
		if (type == 1) {
